...
function format_mapping_InfiniumI {
  ...
  ## reformat A
  grep '^cg' sam/${species}.sam | awk '$2<256 && $1~/_1$/' | awk -F" " -v OFS="\t" '!and($2,0x4) && !and($2,0x10){print $3,$4+47,$4+49,substr($10,50,1),$0} !and($2,0x4) && and($2,0x10){print $3,$4-1,$4+1,substr($10,1,1),$0;} and($2,0x4){print "*",0,0,substr($10,50,1),$0;}' | awk -v OFS="\t" '!and($6,0x4) && (($4=="A" && !and($6,0x10)) || ($4=="T" && and($6,0x10))){$5=substr($5,1,length($5)-2);print $0;}' | awk -f wanding.awk -e '{if(!match(joinr(16,NF),/(NM:[^[:space:]]*)/,nm)){nm1=".";} else nm1=nm[1]; match(joinr(16,NF),/(AS:[^[:space:]]*)/,as); match($0,/(YD:[^[:space:]]*)/,yd); print joinr(1,10),$14,nm1,as[1],yd[1];}' >tmp/${species}_A
    
  ## reformat B
  grep '^cg' sam/${species}.sam | awk '$2<256 && $1~/_2$/' | awk -F" " -v OFS="\t" '!and($2,0x4) && !and($2,0x10){print $3,$4+47,$4+49,substr($10,50,1),$0} !and($2,0x4) && and($2,0x10){print $3,$4-1,$4+1,substr($10,1,1),$0;} and($2,0x4){print "*",0,0,substr($10,50,1),$0;}' | awk -v OFS="\t" '!and($6,0x4) && (($4=="G" && !and($6,0x10)) || ($4=="C" && and($6,0x10))){$5=substr($5,1,length($5)-2);print $0;}' | awk -f wanding.awk -e '{if(!match(joinr(16,NF),/(NM:[^[:space:]]*)/,nm)){nm1=".";} else nm1=nm[1]; match(joinr(16,NF),/(AS:[^[:space:]]*)/,as); match($0,/(YD:[^[:space:]]*)/,yd); print joinr(1,10),$14,nm1,as[1],yd[1];}' >tmp/${species}_B
