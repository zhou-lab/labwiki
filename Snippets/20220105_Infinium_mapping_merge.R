...
mergeI_and_II_1 <- function(x, goodprobes = NULL) {
  ...
  df44 <- read_tsv("tmp/controls.tsv", show_col_types=FALSE, progress=FALSE, col_names=c("Probe_ID", "address_A", "address_B", "col", "type"))
  df44$species <- x
...
mergeI_and_II <- function(dir, goodprobes=NULL) {
  ...
  ns <- simplify2array(mclapply(df$species, function(x) nrow(read_tsv(sprintf('mapping/%s.tsv.gz', x), show_col_types=FALSE, progress=FALSE)), mc.cores=10))
  names(ns) <- df$species
